...
int _StartRoom(char **split_buf)
{
	...
	sprintf_s(query, "UPDATE roominfo SET Room_State=1 WHERE Room_Number=%d",
		atoi(split_buf[0]));
	retval = mysql_query(connection, query);
	if (retval != 0)
	{
...
int _GetUserInfo(char **split_buf, char *SendBuf)
{
	...
	if ((sql_row = mysql_fetch_row(sql_result)) != NULL)    // Result Set에서 1개씩 배열에 가져옴
	{
		...
		else
		{
			...
			char t[5] = "null";
			memcpy(SendBuf + idx_retBuf++, &len, 1);
			memcpy(SendBuf + idx_retBuf, t, len);
			idx_retBuf += len;
		}
		i += 2;
...
int _GetUserListInRoom(char **split_buf, char *retSendBuf)
{
	...
	sprintf_s(query, "SELECT User_Id From user_room Where Room_Number=(SELECT Room_Number From user_room Where User_Id='%s')", split_buf[0]);
	retval = mysql_query(connection, query);
	if (retval != 0)
	{
		ViewMySQLError(connection); return DB_FAIL;
...
int _LeaveRoom(char **split_buf, char *SendBuf)
{
	...
	//////////////////////////////////////////////////////////////////////////
	sprintf_s(query, "UPDATE user_room SET Room_Number=%d, bAgree=%d WHERE User_Id='%s'",
		0, 0, split_buf[0]);
	retval = mysql_query(connection, query);
	if (retval != 0)
	{
	...
	//////////////////////////////////////////////////////////////////////////
	sprintf_s(query, "SELECT User_Id From user_room Where Room_Number=%d", roomNum);
	retval = mysql_query(connection, query);
	if (retval != 0)
	{
		ViewMySQLError(connection); return DB_FAIL;
...
int _AgreeInStage(char **split_buf)
{
	...
	sprintf_s(query, "UPDATE SET bAgree=1 WHERE User_Id='%s'",
		split_buf[0]);
	retval = mysql_query(connection, query);
	if (retval != 0)
	{
...
int _DisagreeInStage(char **split_buf)
{
	...
	sprintf_s(query, "UPDATE SET bAgree=2 WHERE User_Id='%s'",
		split_buf[0]);
	retval = mysql_query(connection, query);
	if (retval != 0)
	{
...
{
	// CMD(2) : Length(2)
	// : lenRoomNum(1) : RoomNumber;
	int retval;

	sprintf_s(query, "SELECT * From roomresultinfo Where Room_Number=%s", split_buf[0]);
	retval = mysql_query(connection, query);
	if (retval != 0)
	{
		ViewMySQLError(connection); return DB_FAIL;
	...
	sprintf_s(query, "UPDATE SET RoomResult_Result=%d WHRER Room_Number=%s", 
