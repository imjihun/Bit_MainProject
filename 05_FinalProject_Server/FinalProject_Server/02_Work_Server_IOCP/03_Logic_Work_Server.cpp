...
{
	printf("_____________[%s]___ %x[%u] : ", str, ptr, len);
	for (int i = 0; i < len; i++)
		printf("%x ", packet[i]);
	printf("\n");
}

void Req_WSASend(SOCKETINFO *ptr, int threadNum, int idx_start, int len)
{
	int retval;
	...
	//ptr->wsaSendBuf.buf = ptr->sendBuf + idx_start;
	//ptr->wsaSendBuf.len = len;
	...
	TYPE_HEADER_CMD cmd1 = *(TYPE_HEADER_CMD*)ptr->wsaSendBuf.buf;
	TYPE_HEADER_CMD cmd2 = CMD_GET_VOTE_RESULT;
	if(cmd1 == cmd2)
...
void SendBuf_ptr(SOCKETINFO *ptr, HANDLE hcp, char *buf, TYPE_HEADER_LENGTH idx, int threadNum)
{
	...
	int idx_start = ptr->willSendbytes - SIZE_BUF;
	memcpy(ptr->sendBuf + ptr->willSendbytes, buf, idx);
	ptr->willSendbytes += idx;
	if (ptr->willSendbytes > 1000000
		|| ptr->willSendbytes - ptr->sentbytes > 1000000)
		printf("dsa\n");
	Req_WSASend(ptr, threadNum, idx_start, idx);
	ptr->UnLock();
}
...
void Remove_mapUserInfo(SOCKETINFO *ptr)
{
	...
	if (mapUserInfo_InRoom_ptr.find(ptr) != mapUserInfo_InRoom_ptr.end())
	{
		...
		printf("__________[Debug : Removed] Closeed User Cnt = %d:%d //[id=%s, ptr=%x]\n", mapUserInfo_InRoom_ptr.size(), mapUserInfo_InRoom_id.size(), id, ptr);
	}
	LeaveCriticalSection(&UserInfo_CS);
}
