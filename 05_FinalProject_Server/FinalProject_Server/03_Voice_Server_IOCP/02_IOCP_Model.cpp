...
void IOCP_SendViewMessage_voice(SOCKETINFO *ptr, int len, int threadNum)
{
	...
	ViewMessage("IOCP[%d] : [TCP/%s:%d] [WillSend_Re : %c%c] [len = %u / %u]", threadNum,
		inet_ntoa(clientaddr.sin_addr),
...
void Req_WSASend_Re(SOCKETINFO *ptr, int threadNum)
{
	int retval;
...
void DataProcess(char *threadBuf, HANDLE hcp, DWORD cbTransferred, SOCKETINFO* ptr, LPOVERLAPPED lpResultOverlapped, int threadNum)
{
	...
	else
	{
		...
		{
			// 데이터 더 보내기
			printf("___________________________________________Re\n");
			Req_WSASend_Re(ptr, threadNum);
